...
async function mcp(body: unknown, env: Env = {}, extraHeaders: Record<string, string> = {}): Promise<Response> {
  return worker.fetch(
...
describe("Cloudflare Worker entrypoint", () => {
  ...
  it("handles MCP initialize", async () => {
    ...
    expect(body.result?.serverInfo?.name).toBe("liongard-mcp");
  });
  ...
  it("lists all tools without credentials", async () => {
    ...
    expect(names).toContain("liongard_navigate");
    expect(names).toContain("liongard_environments_list");
  ...
  it("returns a graceful error for a credential-requiring tool when unconfigured", async () => {
    const res = await mcp({
      ...
      id: 3,
      method: "tools/call",
      params: { name: "liongard_environments_list", arguments: {} },
    });
    expect(res.status).toBe(200);
