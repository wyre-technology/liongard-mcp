/**
 * Cloudflare Workers entry point for the Liongard MCP Server.
 *
...
export interface Env {
  LIONGARD_API_KEY?: string;
  LIONGARD_INSTANCE?: string;
  AUTH_MODE?: string;
  LOG_LEVEL?: string;
}
...
const CORS_HEADERS: Record<string, string> = {
  ...
  "Access-Control-Allow-Headers":
    "Content-Type, Accept, Authorization, Mcp-Session-Id, MCP-Protocol-Version, X-Liongard-API-Key, X-Liongard-Instance",
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/mcp") {
      ...
      let credOverrides: LiongardCredentials | undefined;
      if (isGatewayMode) {
        ...
        const { creds } = buildCredentials(
          env.LIONGARD_API_KEY,
          env.LIONGARD_INSTANCE
        );
        credOverrides = creds;
      }
